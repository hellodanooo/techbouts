// components/EmbedMatchesGenerator.tsx
'use client';
...
export default function EmbedMatchesGenerator({ eventId, eventName, promoterId }: EmbedCodeGeneratorProps) {
...
<script>
  (function() {
    ...
    script.src = "${baseUrl}/embeds/matches-loader.js";
    script.async = true;

    script.setAttribute('data-event-id', '${eventId}');
    script.setAttribute('data-container-id', 'techbouts-matches-${eventId}');
  ...
  const generateWordPressCode = () => {
    ...
    return `[techbouts_matches ${attributes}]`;
};
  ...
  return (
    <Card className="w-full">
      <CardHeader>
      <CardTitle>Embed Matches Display</CardTitle>
      </CardHeader>
      <CardContent>
  ...
  Paste this code into any HTML page to embed your matches display.
</p>
          </TabsContent>
        ...
        <div className="mt-6">
          ...
          <div className="border rounded-md p-2 bg-gray-50">
            <iframe 
src={`${baseUrl}/events/${promoterId}/${eventId}/matches/embed${customization.hideHeader ? '?hideHeader=true' : ''}${customization.customColor ? `${customization.hideHeader ? '&' : '?'}color=${customization.primaryColor.replace('#', '')}` : ''}`}
width={width} 
              height={height}
              frameBorder="0"
