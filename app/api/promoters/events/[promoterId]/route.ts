// app/api/promoters/events/[promoterId]/route.ts
...
export async function GET(
  ...
  console.log("API Route: Starting fetch for promoterId:", promoterId);
  let events: EventType[] = [];

  try {
    const eventCalendarRef = doc(db, "events", "events_json");
    const snapshot = await getDoc(eventCalendarRef);
    console.log("API Route Events: Got Firestore snapshot");

    if (snapshot.exists()) {
      const data = snapshot.data() as { events?: Partial<EventType>[] };

      if (data?.events) {
        // Filter events for specific promoter
        events = data.events
          .filter(event => event.promoterId === promoterId)
          .map((event) => {
      ...
      console.log("API Route Events: No snapshot exists");
    }

    console.log(`API Route: Sending ${events.length} events for promoter ${promoterId}`);
    return NextResponse.json({ events });

  } catch (error) {
    const err = error as Error;
    console.error("API Route Events: Error fetching events from Firestore:", err.message);
